...
fill (0);
rectMode(CENTER);
textToPoints = convertLetterToPoints(`"this is the worst we've seen. \n there are no military targets here."`, width/2, height/2-15, width, height, 5, 54, font, CENTER, CENTER);
pop();
...
textSize (54);
textFont(font);
text(`"this is the worst we've seen. \n there are no military targets here."`, width/2, height/2-15, width, height);
...
//even (or corners)
let toGive = (Math.min(inkToSpread-paper[neighbours[i]], maxTransfer/3));
paper[index]-=toGive; 
paper[neighbours[i]]+=toGive;
}
}
